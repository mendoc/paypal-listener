...
export default async (request, context) => {
  ...
  try {
    ...
    console.log("[handlepaypalpayments]", "Body", bodyPlain);
      ...
      if (parsedEmail.match) {
        try {
          ...
          await databaseService.enqueueWhatsAppMessage(
            ...
            parsedEmail.reference ? `paypal-recu:${parsedEmail.reference}` : null
          );
        } catch (err) {
      ...
      if (parsedEmail.match && firestoreService) {
        try {
          const transfer = await initiateUssdTransfer({
            databaseService,
            firestoreService,
            match: parsedEmail.match,
          });
          if (transfer.initiated) {
            await telegramService.sendTransferInitiatedNotification(transfer);
          } else {
            console.log("[handlepaypalpayments]", "transfert non initié:", transfer.reason);
          }
        } catch (err) {
          console.error("[handlepaypalpayments]", "erreur initiation du transfert Airtel Money (non bloquante):", err);
        }
      }
      ...
      if (parsedEmail.internalReference) {
        try {
          ...
          if (result.success) {
            ...
            if (shouldEmitEvent) {
              try {
                await databaseService.enqueueScreenshot(parsedEmail.internalReference, simulation.whatsapp);
              } catch (err) {
                console.error("[handlepaypalpayments]", "erreur mise en file de la capture (non bloquante):", err);
                await telegramService.sendMessage(`❌ Capture de la transaction ${parsedEmail.internalReference} non envoyée au client : à envoyer à la main.`);
              }
              if (firestoreService) {
