    ...
    try {
      ...
      const svgBuffer = Buffer.from(`
        <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
          ...
          <text x="${width / 2}" y="310" font-family="Verdana" font-size="24" fill="#cccccc" text-anchor="middle">${paymentInfo.reference}</text>

          <!-- Carte inférieure -->
          <rect x="30" y="350" width="${width - 60}" height="${height - 365}" rx="10" fill="#222d65"/>

          <!-- Informations -->
          ${this.generateInfoLine("Référence", paymentInfo.internalReference ? paymentInfo.internalReference : paymentInfo.reference, 400)}
